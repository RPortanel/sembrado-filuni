...
const getColorDependencia = (dependencia) => {
  ...
  if (depNormalizada === 'rector' || depNormalizada === 'rectores') return '#9333ea'; 
...
const getExcelStyle = (dependencia) => {
  ...
  if (!dependencia) return { ...baseStyle, fill: { fgColor: { rgb: 'CBD5E1' } }, font: { color: { rgb: '000000' } } }; 
  ...
  else if (depNormalizada === 'rector' || depNormalizada === 'rectores') { bg = '9333EA'; text = 'FFFFFF'; }
...
const paletaMesas = [
  { bg: '#fee2e2', border: '#ef4444' }, { bg: '#fef3c7', border: '#f59e0b' },
  { bg: '#dcfce7', border: '#22c55e' }, { bg: '#e0f2fe', border: '#0ea5e9' },
  { bg: '#f3e8ff', border: '#a855f7' }, { bg: '#ffedd5', border: '#f97316' },
  { bg: '#ecfccb', border: '#84cc16' }, { bg: '#ccfbf1', border: '#14b8a6' },
  { bg: '#ede9fe', border: '#6366f1' }, { bg: '#ffe4e6', border: '#f43f5e' },
  { bg: '#fae8ff', border: '#d946ef' } 
];
...
export default function App() {
  ...
  // --- 2. LÓGICA DE RESPALDOS, ELIMINACIÓN Y EDICIÓN ---
  const descargarRespaldo = () => {
    ...
    elementoEnlace.setAttribute("download", `Respaldo_Sembrado_${new Date().toISOString().slice(0,10)}.json`);
  ...
  const eliminarInvitadoDeBanca = (id) => {
    ...
    setAuditorio(prev => ({ ...prev, banca: prev.banca.filter(inv => inv.id !== id) }));
    setComida(prev => ({ ...prev, banca: prev.banca.filter(inv => inv.id !== id) }));
  };
  ...
  const guardarEdicion = (invitadoActualizado) => {
    ...
    setAuditorio(actualizarLayout(auditorio));
    setComida(actualizarLayout(comida));
    setInvitadoEditando(null); 
  };

  // --- 3. LÓGICA DE EXPORTACIÓN A EXCEL ---
  const exportarExcel = () => {
    ...
    Object.keys(auditorio).forEach(key => {
      ...
      let ubicacionStr = key.includes('estrado') 
        ? key.replace('estrado_silla_', 'Estrado - Silla ') 
        : `Fila ${key.split('_')[1]} - Silla ${key.split('_')[3]}`;
    ...
    for(let f=1; f<=10; f++) {
        ...
        for(let s=1; s<=13; s++) {
             if ((f === 5 || f === 6) && (s >= 6 && s <= 8)) {
                 if (s === 7) filaAsientos.push({ v: "[ TV UNAM ]", t: 's', s: { fill: { fgColor: { rgb: "1E293B" } }, font: { color: { rgb: "FFFFFF" }, bold: true }, alignment: { vertical: 'center', horizontal: 'center' } } });
    ...
    const ws3 = XLSX.utils.aoa_to_sheet(matrizAuditorio);
    ws3['!cols'] = Array(16).fill({ wch: 25 }); 
    XLSX.utils.book_append_sheet(wb, ws3, "Gráfico Auditorio");

    const ws4 = XLSX.utils.aoa_to_sheet(matrizComida);
    ws4['!cols'] = Array(6).fill({ wch: 25 }); 
    XLSX.utils.book_append_sheet(wb, ws4, "Gráfico Comida");
      ...
      try {
        ...
        const nuevosInvitados = filasExcel.map((fila, index) => {
          ...
          return {
            ...
            dependencia: filaNormalizada.dependencia || '',
            nombre: filaNormalizada.nombre || 'Sin Nombre',
            cargo: filaNormalizada.cargo || 'Sin Cargo',
          };
        });

        setAuditorio(prev => ({ ...prev, banca: [...prev.banca, ...nuevosInvitados] }));
        setComida(prev => ({ ...prev, banca: [...prev.banca, ...nuevosInvitados] }));
        alert(`✅ Se cargaron ${nuevosInvitados.length} invitados.`);
        e.target.value = null; 
  ...
  // --- 5. LÓGICA DEL PDF COMPLEMENTARIO Y ESCALADO AL MÁXIMO ---
  const exportarPDF = async () => {
    ...
    const bloquesACapturar = vistaActual === 'auditorio' 
      ? ['auditorio-parte-1', 'auditorio-parte-2'] 
      : ['comida-parte-1', 'comida-parte-2', 'comida-parte-3', 'comida-parte-4'];
    ...
    try {
      for (let i = 0; i < bloquesACapturar.length; i++) {
        ...
        const zoomAnterior = element.style.zoom;
        const widthAnterior = element.style.width;
        ...
        element.style.zoom = zoomAnterior; 
        element.style.width = widthAnterior;
        ...
        pdf.text("Inauguración Filuni 2026", 10, 12);

        const margenX = 6;
        const topReserved = 18; 
        const bottomReserved = 12; 
        const maxImgHeight = pdfHeightSheet - topReserved - bottomReserved; 

        let imgWidth = pdfWidth - (margenX * 2);
        ...
        const yOffset = topReserved + (maxImgHeight - imgHeight) / 2;
        ...
        pdf.text(`Fecha de generación: ${fechaHoy}`, pdfWidth - 10, pdfHeightSheet - 7, { align: 'right' });
      }
  ...
  // --- 6. ARRASTRAR, SOLTAR Y REORDENAR ---
  const onDragEnd = (result) => {
    const { source, destination } = result;
    if (!destination) return;

    const estadoActivo = vistaActual === 'auditorio' ? auditorio : comida;
    const setEstadoActivo = vistaActual === 'auditorio' ? setAuditorio : setComida;
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      setEstadoActivo({ ...estadoActivo, [source.droppableId]: nuevaLista });
      return;
    }
    ...
    <div key={`mesa_${m}`} style={{ width: '320px', backgroundColor: paletaMesas[m - 1].bg, border: `3px solid ${paletaMesas[m - 1].border}`, borderRadius: '8px', padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', flexShrink: 0 }}>
      <div style={{ width: '100%', marginBottom: '15px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <input 
          ...
          onChange={(e) => setNombresMesas({...nombresMesas, [`mesa_${m}`]: e.target.value})} 
          style={{ 
            width: '100%', 
            height: '40px',
            lineHeight: '36px',
    ...
    <div style={{ display: 'flex', height: '100vh', width: '100vw', fontFamily: 'sans-serif', backgroundColor: '#f1f5f9', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 99999, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '8px', width: '350px', display: 'flex', flexDirection: 'column', gap: '15px', boxShadow: '0 10px 25px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
              ...
              <input type="text" value={invitadoEditando.dependencia} onChange={(e) => setInvitadoEditando({...invitadoEditando, dependencia: e.target.value})} style={{ padding: '8px', borderRadius: '4px', border: '1px solid #cbd5e1', outline: 'none' }} placeholder="Ej: UNAM, UV, Externo, DGPyFE" />
            </div>

            <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
              <button onClick={() => guardarEdicion(invitadoEditando)} style={{ flex: 1, backgroundColor: '#3b82f6', color: 'white', padding: '10px', borderRadius: '4px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>Guardar</button>
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        ...
        <div style={{ width: '320px', flexShrink: 0, backgroundColor: 'white', padding: '15px', borderRight: '1px solid #cbd5e1', display: 'flex', flexDirection: 'column', zIndex: 10 }}>
          <h2 style={{ fontSize: '18px', fontWeight: 'bold', marginBottom: '15px' }}>Panel de Control</h2>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '15px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={descargarRespaldo} style={{ flex: 1, backgroundColor: '#6366f1', color: 'white', padding: '8px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' }}>
                💾 Descargar Avance
              </button>
              <label style={{ flex: 1, backgroundColor: '#f59e0b', color: 'white', padding: '8px', textAlign: 'center', borderRadius: '4px', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' }}>
                📂 Subir Avance
            ...
            {vistaActual === 'auditorio' && (
              <div id="contenedor-auditorio" style={{ width: '100%', overflowX: 'auto', paddingBottom: '20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 'max-content', padding: '0 20px' }}>
                  ...
                  {/* HOJA 1: Estrado (7 asientos) + Numeración + Filas 1 a 6 */}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '0 15px', height: '30px', width: '100%' }}>
                      ...
                      <div style={{ display: 'flex', gap: '10px' }}>
                        {Array.from({ length: 13 }, (_, sIndex) => (
                          <div key={`num_guia_b1_${sIndex+1}`} style={{ width: '120px', minWidth: '120px', textAlign: 'center', color: '#64748b', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                            Asiento {sIndex + 1}
                          </div>
                        ))}
                      </div>
                          ...
                          <div key={`fila_${f}`} style={{ display: 'flex', alignItems: 'flex-start', gap: '15px', backgroundColor: '#f8fafc', padding: '15px', borderRadius: '6px', border: '1px solid #e2e8f0', height: '125px', boxSizing: 'border-box' }}>
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                              {Array.from({ length: 13 }, (_, sIndex) => {
                                ...
                                if (f === 5 && s === 6) {
                                  return (
                                    <div key="tv_unam" style={{ position: 'relative', width: '380px', height: '95px', flexShrink: 0 }}>
                                      <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '240px', backgroundColor: '#1e293b', color: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '8px', fontWeight: 'bold', fontSize: '18px', letterSpacing: '2px', zIndex: 9999, boxShadow: '0 4px 6px -1px rgba(0,0,0,0.3)' }}>
                                        📺 TV UNAM
                                      </div>
                                    </div>
                                  );
                  ...
                  {/* PASILLO CENTRAL */}
                  <div style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '30px 0', height: '45px', backgroundColor: '#cbd5e1', borderRadius: '6px', border: '2px dashed #94a3b8' }}>
                    <span style={{ fontWeight: 'bold', letterSpacing: '12px', color: '#475569', fontSize: '16px' }}>P A S I L L O</span>
                  </div>

                  {/* HOJA 2: Numeración Superior + Filas 7 a 10 */}
                  <div id="auditorio-parte-2" style={{ display: 'flex', flexDirection: 'column', gap: '20px', width: '100%', alignItems: 'center', backgroundColor: 'white' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '0 15px', height: '30px', width: '100%' }}>
                      ...
                      <div style={{ display: 'flex', gap: '10px' }}>
                        {Array.from({ length: 13 }, (_, sIndex) => (
                          <div key={`num_guia_b2_${sIndex+1}`} style={{ width: '120px', minWidth: '120px', textAlign: 'center', color: '#64748b', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                            Asiento {sIndex + 1}
                          </div>
                        ))}
                      </div>
            ...
            {vistaActual === 'comida' && (
              <div id="contenedor-comida" style={{ width: '100%', overflowX: 'auto', paddingBottom: '20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '40px', alignItems: 'center', minWidth: 'max-content', padding: '0 20px' }}>
                  ...
                  <div id="comida-parte-1" style={{ display: 'flex', flexDirection: 'row', flexWrap: 'nowrap', justifyContent: 'center', gap: '40px', padding: '20px', backgroundColor: 'white', borderRadius: '8px' }}>
                    {Array.from({ length: 3 }, (_, i) => renderMesaLayout(i + 1))}
                  </div>

                  <div id="comida-parte-2" style={{ display: 'flex', flexDirection: 'row', flexWrap: 'nowrap', justifyContent: 'center', gap: '40px', padding: '20px', backgroundColor: 'white', borderRadius: '8px' }}>
                    {Array.from({ length: 3 }, (_, i) => renderMesaLayout(i + 4))}
                  </div>

                  <div id="comida-parte-3" style={{ display: 'flex', flexDirection: 'row', flexWrap: 'nowrap', justifyContent: 'center', gap: '40px', padding: '20px', backgroundColor: 'white', borderRadius: '8px' }}>
                    {Array.from({ length: 3 }, (_, i) => renderMesaLayout(i + 7))}
                  </div>

                  <div id="comida-parte-4" style={{ display: 'flex', flexDirection: 'row', flexWrap: 'nowrap', justifyContent: 'center', gap: '40px', padding: '20px', backgroundColor: 'white', borderRadius: '8px' }}>
                    {Array.from({ length: 2 }, (_, i) => renderMesaLayout(i + 10))}
                  </div>
...
function Silla({ id, ocupante, vista, busqueda, onEdit }) {
  const isAuditorio = vista === 'auditorio';
  const width = isAuditorio ? '120px' : '100%';
  const minWidth = isAuditorio ? '120px' : '85px';
  const height = isAuditorio ? '95px' : '75px';
  const flexShrink = isAuditorio ? 0 : 1;
  const arrOcupante = Array.isArray(ocupante) ? ocupante : [];

  return (
    <Droppable droppableId={id}>
      {(provided) => (
        <div ref={provided.innerRef} {...provided.droppableProps} style={{ 
          width, minWidth, height, flexShrink, 
          border: arrOcupante.length === 0 ? '2px dashed #94a3b8' : 'none', 
          backgroundColor: arrOcupante.length === 0 ? 'rgba(255,255,255,0.5)' : 'transparent', 
          borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxSizing: 'border-box'
        }}>
...
function Tarjeta({ invitado, index, enSilla, isBanca, onDelete, busqueda, onEdit }) {
  const colorBorde = getColorDependencia(invitado.dependencia);
  const textoBusqueda = busqueda ? busqueda.toLowerCase() : '';
  const coincideBusqueda = textoBusqueda !== '' && (
    invitado.nombre.toLowerCase().includes(textoBusqueda) || 
    invitado.cargo.toLowerCase().includes(textoBusqueda) || 
    invitado.dependencia.toLowerCase().includes(textoBusqueda)
  );
  
  const opacity = (textoBusqueda !== '' && !coincideBusqueda) ? 0.2 : 1;
    ...
    <Draggable draggableId={invitado.id} index={index}>
      {(provided) => (
        <div 
          ...
          onDoubleClick={() => onEdit(invitado)}
          title="Doble clic para editar"
          style={{ 
            userSelect: 'none', padding: '6px', backgroundColor: 'white', border: '1px solid #cbd5e1', 
            borderTop: `4px solid ${colorBorde}`, borderRadius: '4px', boxShadow: glow, transform: provided.draggableProps.style?.transform || escala,
            width: enSilla ? '100%' : '100%', height: enSilla ? '100%' : 'auto', marginBottom: enSilla ? '0' : '10px', 
            display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', 
            boxSizing: 'border-box', position: 'relative', transition: 'box-shadow 0.3s, opacity 0.3s', opacity, ...provided.draggableProps.style 
          }}
        >
          {/* Botón de Eliminación (Solo Banca) */}
          {isBanca && (
            <button onPointerDown={(e) => e.stopPropagation()} onClick={() => onDelete(invitado.id)} title="Eliminar invitado" style={{ position: 'absolute', top: '-6px', right: '-6px', width: '18px', height: '18px', backgroundColor: '#ef4444', color: 'white', border: 'none', borderRadius: '50%', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 4px rgba(0,0,0,0.2)' }} >×</button>
          )}
